...
export function getProceduralQuestion(subjectId: string, difficulty: string, targetExamDate?: string, cfaLevel?: number): Question {
  ...
  switch (subjId) {
    case "quant":
      quest = generateQuantQuestion(randomId, difficulty);
      break;
    case "econ":
      quest = generateEconQuestion(randomId, difficulty);
      break;
    case "portfolio":
      quest = generatePortfolioQuestion(randomId, difficulty);
      break;
    case "corporate":
      quest = generateCorporateQuestion(randomId, difficulty);
      break;
    case "fsa":
      quest = generateFsaQuestion(randomId, difficulty);
      break;
    case "equity":
      quest = generateEquityQuestion(randomId, difficulty);
      break;
    case "fixed":
      quest = generateFixedIncomeQuestion(randomId, difficulty);
      break;
    case "derivatives":
      quest = generateDerivativesQuestion(randomId, difficulty);
      break;
    case "alt":
      quest = generateAltInvestmentsQuestion(randomId, difficulty);
      break;
    case "ethics":
    default:
      quest = generateEthicsQuestion(randomId, difficulty);
      break;
  }
  ...
  if (level === 2) {
    ...
    if (subj2026 && subj2027) {
      const idx = subj2026.modules.findIndex(m => m.id === quest.moduleId);
      if (idx !== -1) {
        const mappedMod = subj2027.modules[idx] || subj2027.modules[subj2027.modules.length - 1];
        if (mappedMod) {
          quest.moduleId = mappedMod.id;
        }
      } else {
        const fallbackMod = subj2027.modules[0];
        if (fallbackMod) {
          quest.moduleId = fallbackMod.id;
        }
      }
    }
  }
