...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    function doResize() {
      if (!canvas) return;
      const rect = canvas.parentElement?.getBoundingClientRect() || { width: 0, height: 0, left: 0, top: 0 };
      const w = rect.width;
      const h = rect.height;
      ...
      sizeRef.current = {
        w,
        h,
        offsetX: (rect.left || 0) + window.scrollX,
        offsetY: (rect.top || 0) + window.scrollY,
      };
    ...
    function buildDots(w: number, h: number) {
      ...
      const padX = (w % step) / 2;
      const padY = (h % step) / 2;
      const dots = new Array<DotProps>(rows * cols);
      let idx = 0;
